...
func _physics_process(delta: float) -> void:
	...
	#attacks
	if Input.is_action_just_pressed("attack_2") and Input.is_action_pressed("move_down_2") and not is_on_floor() and is_stunned == false :
		down_tilt_attack()
		return
	elif Input.is_action_just_pressed("attack_2") and is_stunned == false:
		attack()
		return
	#SP attack
	if Input.is_action_just_pressed("SP_2") and is_on_floor() and is_stunned == false:
		...
		return
	#SP right
	if Input.is_action_just_pressed("SP_RIGHT_2") and is_on_floor() and is_stunned == false:
		attack_SP2()
		return
...
func die():
	...
	print("Player2 died! Lives left: " + str(player_lives))
	
	if player_lives <= 0:
		print("PLAYER 2 LOST THE ROUND!")
		
		# Toon win screen kort
		P1_win.visible = true
...
func respawn():
	...
	global_position = Vector2(600, 100)  # Andere spawn positie dan P1!
	velocity = Vector2.ZERO
	is_stunned = false
	is_attacking = false
	# player_hp wordt gereset door game.gd
...
func attack():
	...
	for body in hit_bodies:
		if body != self and body.has_method("get_stunned"):
			...
			print("Player1 HP: " + str(player_hp))
...
func normal_SP():
	...
	for body in hit_bodies:
		if body != self and body.has_method("get_stunned"):
			...
			SP = true    
			if last_diraction == 1:
				body.get_stunned(last_diraction, Player_DMG,false,true,false,false,false)
			if last_diraction == -1:
				body.get_stunned(last_diraction, Player_DMG,false,true,false,false,false)
...
func get_stunned(knockback_direction, damage = 5, is_normal = true ,is_SP1 = false , is_SP2 = false, is_SP3 = false, is_SP4 = false):  
	...
	if is_normal:
		...
		velocity.y = -100
		player_hp += (damage*Player_DMG_muliplyer)
		update_percentage_ui()
		...
		for n in 8:
			...
			velocity.x = knockback_direction * knockback_force
			velocity.y = -100
			player_hp += (1*Player_DMG_muliplyer)
			update_percentage_ui()
			print("Got hit! Damage: " + str(damage))
			print("My HP: " + str(player_hp))
